...
describe("Test missing numeric sentinel formatters", () => {
  it("should detect -999 as missing", () => {
    expect(isMissingNumericValue(-999)).toBe(true);
    expect(isMissingNumericValue("-999")).toBe(true);
    expect(isMissingNumericValue(0)).toBe(false);
    expect(isMissingNumericValue(null)).toBe(false);
  });

  it("should format -999 as dash", () => {
    expect(formatNumericOrDash(-999)).toBe("-");
    expect(formatNumericOrDash(12.5)).toBe(12.5);
    expect(formatPercentageOrDash(-999)).toBe("-");
    expect(formatPercentageOrDash(42)).toBe("42%");
    expect(formatPercentageOrDash("42%")).toBe("42%");
  });
});

describe("Test the hasValue utility function", () => {
  it("should reject null, undefined, empty string, em dash string, empty array", () => {
  ...
  it("should accept meaningful values including zero and false", () => {
...
describe("fileVersionSortKey", () => {
  it("orders version strings by numeric portion", () => {
    const versions = ["v10", "v2", "v1", "v4", "v3"];
    const sorted = versions.sort((a, b) => {
      const [aNum, aStr] = fileVersionSortKey(a);
      const [bNum, bStr] = fileVersionSortKey(b);
      return aNum - bNum || aStr.localeCompare(bStr);
    });
    expect(sorted).toEqual(["v1", "v2", "v3", "v4", "v10"]);
  });

  it("sorts missing versions last", () => {
    expect(fileVersionSortKey("v2")[0]).toBeLessThan(fileVersionSortKey(null)[0]);
    expect(fileVersionSortKey("v2")[0]).toBeLessThan(fileVersionSortKey("")[0]);
  });
});
