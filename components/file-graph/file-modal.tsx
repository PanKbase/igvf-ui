      ...
      <Modal.Header onClose={onClose as any}>
        <FileModalTitle item={file}>
          <div className="flex items-center gap-1">
            <Link href={file["@id"] as string} target="_blank" rel="noopener noreferrer">
              {file.accession}
            </Link>
        ...
        <DataArea>
          ...
          {file.lab && (
            <>
              ...
              <DataItemValue>
                <Link
                  href={((file.lab as DatabaseObject)["@id"]) as string}
                  target="_blank"
          ...
          <DataItemValue className="flex items-center gap-1">
            <Status status={file.status as string} />
            <Status status={file.upload_status as string} />
          </DataItemValue>
          {file.derived_from && file.derived_from.length > 0 && (
            <>
              <DataItemLabel>
