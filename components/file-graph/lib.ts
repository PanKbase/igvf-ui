...
export function generateGraphData(
  ...
  const nativeFilePaths = nativeFiles.map((file) => file["@id"] as string);
  ...
  const graphData: NodeData[] = nativeFiles.map((nativeFile) => {
    ...
    return {
      ...
      externalFiles: derivedFromFiles.filter(
        (derivedFile) => {
          const derivedFileId = derivedFile["@id"] as string;
          return nativeFile.derived_from ? nativeFile.derived_from.includes(derivedFileId) : false;
        }
      ),
    } as FileNodeData;
  });
  ...
  // derive from.
  const fileSetNodes = graphData
    .filter(isFileNodeData) // Filter to ensure we only have FileNodeData elements
    .reduce((acc: FileSetNodeData[], fileNode: FileNodeData) => {
      ...
      if (fileNode.externalFiles.length > 0) {
        ...
        const fileSetGroups = _.groupBy(
          ...
          (file: { file_set: { ["@id"]: string } }) => file.file_set["@id"]
        );

        // For each file set group, create a file set node and add it to the list of nodes.
        nodes = Object.entries(fileSetGroups).reduce(
          (fileSetNodeAcc: FileSetNodeData[], [fileSetPath, files]) => {
            const fileSet = fileFileSets.find(
              (fs) => fs["@id"] === fileSetPath
            );

            if (!fileSet) return fileSetNodeAcc; // Add a check to handle potential undefined
            ...
            const combinedFilePaths = (files as FileObject[])
